...
def missed_poop_check(db):
  try:
    ...
    for member in channel.members:
      ...
      if not poop_record and member.id not in (903380664336928798, 1092634707541360762):
...
def make_multi_user_heatmap_grid(poops):
    ...
    for row in poops:
        uid = row['user']
        name = row.get('nickname') or str(uid)
...
async def on_message(message):
  ...
  triggers = {
    ...
    "804804163904340029": {"type": "reaction", "value": "mynameisjeff:1096781925114466405"},
    "berry": {"type": "reaction", "value": "berry:1096783181228814438"},
    "462087982523088908": {"type": "reaction", "value": "berry:1096783181228814438"},
    "chuggy": {"type": "reaction", "value": "chuggy:1148715141651763270"},
    "284719233601110016": {"type": "reaction", "value": "chuggy:1148715141651763270"},
    "alex": {"type": "reaction", "value": "\N{TRUMPET}"},
    "1323792156825223279": {"type": "reaction", "value": "\N{TRUMPET}"},
